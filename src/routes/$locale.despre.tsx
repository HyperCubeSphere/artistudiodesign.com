...
function DesprePage() {
  ...
  return (
    <>
      ...
      <section className="border-t hairline py-20 md:py-24 bg-surface">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10">
          ...
          <ul className="border-t hairline">
            {d.values.map((v, i) => (
              ...
              >
                <span className="serif text-3xl md:text-5xl tabular-nums leading-none text-accent">
                  {pad2(i + 1)}
                </span>
                <h3 className="serif text-2xl md:text-3xl leading-tight col-start-2 md:col-start-2">
                  {v.title}
                </h3>
                <p className="text-base leading-relaxed col-start-2 md:col-start-3 md:row-start-1 max-w-prose text-muted">
                  {v.description}
                </p>
              </li>
            ))}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 border-t hairline">
            {d.stats.map((s) => (
              <div key={s.label} className="py-8 px-2 border-r last:border-r-0 hairline">
                <p className="serif text-5xl md:text-6xl text-accent tabular-nums mb-2">{s.value}</p>
                <p className="nav-text text-xs text-muted">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
